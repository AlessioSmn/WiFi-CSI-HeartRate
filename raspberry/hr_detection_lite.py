...
def prediction_thread(port):
    ...
    while not stop_event.is_set():
        ...
        # preparazione input
        input_data = np.array(window, dtype=np.float16)  # usa float32 o float16 a seconda della quantizzazione
        input_data = np.expand_dims(input_data, axis=0)  # batch dimension
        # fai predizione
        interpreter.set_tensor(input_details[0]['index'], input_data)
        interpreter.invoke()
        new_prediction = interpreter.get_tensor(output_details[0]['index'])
