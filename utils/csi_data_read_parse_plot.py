...
def csi_data_source_init(port):
    global ser, csv_array
    ...
    if SOURCE_SERIAL:
        ...
        with open(port, newline='') as csvfile:
            ...
            print(f"csv lines: {len(full_data)}")
            for data_row in full_data:
                csv_array.append(data_row)


index = 0
def get_csi_data(log_file_fd):
    global index, csv_array, ser
    frequency_hz = 40
    ...
    if SOURCE_SERIAL:
        ...
        # Format csv data in the same way it's coming from serial line

        print("get_csi_data - Reading line of index {0}".format(index))
            
        csi_cvs_row = csv_array[index]
        return csi_cvs_row
...
def csi_data_read_parse(port: str, csv_writer, log_file_fd,callback=None):
    ...
    csi_data_source_init(port)
    # TESTING: using port as .csv source file
